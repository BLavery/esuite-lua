...
-- Publish LIGHT level every 20 seconds. 
...
function mqtt_recv(topic,data)
        ...
        if topic == "testButn" then 
            mqtt_publish("Button", gpio.read(3)) 
            -- "testButn" is a poll request. publish current D3 status as a reply.
        end      
end
